...
int yylex(void);             // Declare lexer function prototype
int yyerror(char *s);        // Declare error function prototype
...
int ind = 0;
int flag = 0;
char temp = 'T'; // Start temp variable names at 'T'
...
char AddToTable(char opd1, char opd2, char opr) {
    ...
    char retTemp = temp;
    if (temp < 'Z') {
        temp++;  // Move to next temp variable name
    }
    return retTemp;
}
